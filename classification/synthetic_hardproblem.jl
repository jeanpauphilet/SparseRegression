...
for ARG in ARGS
    ...
    for experiment in 1:3
        ...
        nRange = (experiment % 3 == 1) ? collect(500:500:8500) : ((experiment % 3 == 2) ? collect(500:600:9500) : collect(500:1000:15000))
        if sense > .5
        ...
        if n_index <= length(nRange)
            ...
            for n in [nRange[n_index]]
                ...
                println("***********************")
                @time X, Y, indices_true, w_true = hard_data_construction(SubsetSelection.L1SVM(), n+400, k, p, snr, ρ)
