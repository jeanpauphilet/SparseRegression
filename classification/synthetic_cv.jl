...
do_lasso = true; do_enet = true #Chooses which method to compute
    ...
    for experiment in 1:6
        ...
        if n_index <= length(nRange)
            ...
            for n in nRange[n_index]
                ...
                X, Y, indices_true, w_true = data_construction(SubsetSelection.L1SVM(), n+700, k_true, p, snr, ρ)
                ...
                if do_mcp
                    ...
                    for gamma in logspace(log10(2), log10(4.5), 7)
                        tic()
                        R"mcp = ncvreg($x, $y, family=\"binomial\", penalty=\"MCP\",
                                                                gamma=$gamma, alpha=1, returnX=FALSE)" #lambda.min=.001
                        Δt_mcp = toc()
                        @rget mcp
                ...
                if do_scad
                    ...
                    for gamma in logspace(log10(3), log10(4.5), 7)
                        tic()
                        R"scad = ncvreg($x, $y, family=\"binomial\", penalty=\"SCAD\",
                                                                gamma=$gamma, alpha=1, returnX=FALSE)"
                        Δt_scad = toc()
                        @rget scad
