...
for ARG in ARGS
    ...
    for experiment in 1:1:6

        p = (experiment % 3 == 1) ? 10000 : ((experiment % 3 == 2) ? 5000 : 1000)
        k = (experiment % 3 == 1) ? 100 : ((experiment % 3 == 2) ? 50 : 10)

        nRange = (experiment % 3 == 1) ? collect(500:400:6500) : ((experiment % 3 == 2) ? collect(500:500:7500) : collect(500:700:11700))
        if sense > .5
            sort!(nRange, rev=true)
        end

        n_treshold = (experiment % 3 == 1) ? 4600 : ((experiment % 3 == 2) ? 5500 : 8200)

        if n_index <= length(nRange) && nRange[n_index] < n_treshold
            prefix=string("Fix_", EXPERIMENTS[:prefix][experiment], "_")
            ...
            for n in [nRange[n_index]]
                ...
                println("***********************")
                X, Y, indices_true, w_true = data_construction(SubsetSelection.L1SVM(), n+400, k, p, snr, ρ)
                ...
                if do_mcp
                  for gamma in logspace(log10(2), log10(4.5), 7)
                    tic()
                    R"mcp = ncvreg($x, $y, family=\"binomial\", penalty=\"MCP\", gamma=$gamma, alpha=1, returnX=FALSE)"
                    Δt_mcp = toc()
                    @rget mcp
                ...
                if do_scad
                    for gamma in logspace(log10(3), log10(4.5), 7)
                      tic()
                      R"scad = ncvreg($x, $y, family=\"binomial\", penalty=\"SCAD\", gamma=$gamma, alpha=1, returnX=FALSE)"
                      Δt_scad = toc()
                      @rget scad
                ...
                if do_saddle||do_cio
                  ...
                  γ0 = p /k /(maximum(sum(X[train,:].^2,2))*n)
                  factor = 1. / 2
                  ...
                  for inner_epoch in 1:10
                      if !stop
                          ...
                          indicesRelax=shuffle(1:p)[1:k] #find(x-> x< k/size(X,2), rand(size(X,2)))
                          if do_saddle
                              tic()
                              result_saddle = subsetSelection(ℓ, Constraint(k), Y[train], X[train,:], γ=γ, noImprov_threshold=200)
                              Δt_saddle = toc()
                          ...
                          if do_cio
                              indices_cio, w_cio, Δt_cio, status, Gap, cutCount = oa_formulation(ℓ, Y[train], X[train,:], k, γ,
                                    indices0=indicesRelax, ΔT_max=ΔTmax, verbose=true, solver=:CPLEX)
