...
ℓ = SubsetSelection.OLS() #Loss function used
...
for ARG in ARGS #ARGS correspond to the number given in the command julia synthetic_problem.jl xxx
    ...
    n_index = div(array_num, 10) + 1    #Number of the experiment
    ...
    for experiment in 1:1:6
        ...
        if n_index <= length(nRange)
            ...
            for n in [nRange[n_index]]
                ...
                println("***********************")
                X, Y, indices_true, w_true = data_construction(ℓ, n+400, k, p, snr, ρ)
